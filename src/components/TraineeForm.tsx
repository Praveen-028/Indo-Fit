...
import { X, ArrowRight, ArrowLeft, UserPlus, Users, Calendar } from 'lucide-react';
...
export const TraineeForm: React.FC<TraineeFormProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    membershipStartDate: new Date().toISOString().split('T')[0], // NEW: Editable admission date
    invoiceDate: new Date().toISOString().split('T')[0], // NEW: Editable invoice date
  });
  ...
  useEffect(() => {
    if (isOpen && editingTrainee) {
      ...
      setFormData({
        ...
        membershipStartDate: new Date(editingTrainee.membershipStartDate).toISOString().split('T')[0],
        invoiceDate: editingTrainee.invoiceDate ? new Date(editingTrainee.invoiceDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      });
      setStep(1);
  ...
  const calculateMembershipEndDate = (startDate: string, duration: number) => {
  ...
  const handleSubmit = async () => {
    ...
    if (startDate > new Date()) {
      // Allow future start dates but show warning
    ...
    try {
      if (editingTrainee) {
        ...
        const membershipEndDate = calculateMembershipEndDate(formData.membershipStartDate, formData.membershipDuration);
        ...
        // Add new trainee
        const startDate = new Date(formData.membershipStartDate);
        const endDate = calculateMembershipEndDate(formData.membershipStartDate, formData.membershipDuration);
          ...
          {step === 1 ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Membership end date will be calculated based on this date and duration
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  This date will appear on generated invoices
                </p>
              </div>
            </div>
          ) : (
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-blue-600 mt-1">
                  Membership will expire on: {calculateMembershipEndDate(formData.membershipStartDate, formData.membershipDuration).toLocaleDateString()}
                </p>
              </div>
